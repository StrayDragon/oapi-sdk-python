from lark_oapi.remaintain.extra import DOMAIN_FEISHU, Config, LEVEL_DEBUG
from lark_oapi.remaintain.extra.service.drive_explorer.v2 import (
    Service as DriveExplorerV2Service,
    model as drive_explorer_v2_model,
)
from lark_oapi.remaintain.extra.service.sheets.v2 import (
    Service as SheetsV2Service,
    model as sheets_v2_model,
)
...
def test_create_sheets():
    ...
    resp = (
        driveExplorerV2Service.files.create(fileCreateReqBody)
        .set_folderToken(folderRootMetaResult.token)
        .do()
    )
    ...
    spreadsheetsSheetsBatchUpdateReqBody = (
        sheets_v2_model.SpreadsheetsSheetsBatchUpdateReqBody()
    )
    request = sheets_v2_model.Request(
        add_sheet=sheets_v2_model.AddSheet(
            properties=sheets_v2_model.Properties(title="test_sheet")
        )
    )
    spreadsheetsSheetsBatchUpdateReqBody.requests = [request]

    resp = (
        sheetsV2Service.spreadsheetss.sheets_batch_update(
            spreadsheetsSheetsBatchUpdateReqBody
        )
        .set_spreadsheetToken(sheetsToken)
        .do()
    )
    ...
    spreadsheetsValuesPrependReqBody = sheets_v2_model.SpreadsheetsValuesPrependReqBody(
        value_range=sheets_v2_model.ValueRange(
            range=sheetId + "!A1:C1", values=[[1, 2, "3"]]
        )
    )
    resp = (
        sheetsV2Service.spreadsheetss.values_prepend(spreadsheetsValuesPrependReqBody)
        .set_spreadsheetToken(sheetsToken)
        .do()
    )
