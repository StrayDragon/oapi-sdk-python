...
from lark_oapi.remaintain.extra.service.image.v4 import Service as ImageV4Service

from sample.config.config import (
    test_config_with_memory_store,
    test_config_with_redis_store,
)
from src.lark_oapi.remaintain.extra import DOMAIN_FEISHU, DOMAIN_LARK_SUITE
...
def test_image_get():
    f = open("./aa.png", "wb")
    resp = (
        service.images.get(response_stream=f)
        .set_image_key("img_a3183080-928f-4bb7-b256-177523c762eg")
        .do()
    )
    print("request id = %s" % resp.get_request_id())
    print(resp.code)
...
def test_image_get_bytes():
    resp = (
        service.images.get()
        .set_image_key("img_5b1a1887-5039-4176-b6c3-badc77ce5eag")
        .do()
    )
    print("request id = %s" % resp.get_request_id())
    print(resp.code)
