...
from lark_oapi.remaintain.extra.utils.dt import to_json_decorator, make_datatype
...
@attr.s
class ValueRange:
    major_dimension = attr.ib(
        type=str, default=None, metadata={"json": "majorDimension"}
    )
...
class SpreadsheetsValuesGetResult:
    revision = attr.ib(type=int, default=None, metadata={"json": "revision"})
    spreadsheet_token = attr.ib(
        type=str, default=None, metadata={"json": "spreadsheetToken"}
    )
    value_range = attr.ib(
        type=ValueRange, default=None, metadata={"json": "valueRange"}
    )
