    ...
    def do_request(self, config):  # type: (Config) -> requests.Response
        ...
        if isinstance(body, FormData):
            ...
            if self.response_stream_file:
                ...
                self.response = session.request(
                    method, path, params=params, data=body, timeout=self.timeout
                )
    ...
    def prepare(self):  # type: () -> None
        ...
        if not req.http_path.startswith("http"):
            if req.http_path.startswith("/open-apis/"):
                ...
                req.http_path = "{}/{}/{}".format(
                    conf.domain, OAPI_ROOT_PATH, req.http_path
                )
    ...
    def sign(self):  # type: () -> None
        ...
        if req.need_help_desk_auth:
            if self.config.helpDeskAuthorization:
                req.set_helpdesk_authorization_to_header(
                    self.config.helpDeskAuthorization
                )
            else:
...
class AccessToken:
    ...
    def set_app_access_token_to_store(self, access_token):
        ...
        try:
            ...
            conf.logger.warn(
                "storage app_access_token into storage occurred an error:%s"
                % e.__str__()
            )
    ...
    def set_tenant_access_token_to_store(self, access_token):
        ...
        conf = self.handlers.config
        key = "{}-{}-{}".format(
            TENANT_ACCESS_TOKEN_KEY_PREFIX, conf.app_settings.app_id, tenant_key
        )
        try:
            conf.store.set(key, token, expire - EXPIRE_DELTA)
        except Exception as e:
            conf.logger.warn(
                "storage tenant_access_token into storage occurred an error:%s"
                % e.__str__()
            )
    ...
    def get_internal_app_access_token(self):  # type: () -> AppAccessToken
        ...
        if resp.code != ERR_CODE_OK:
            raise APIError(
                "obtain internal app access token，failure information:%s" % resp
            )
    ...
    def get_isv_tenant_access_token(self):
        ...
        if resp.code != ERR_CODE_OK:
            raise APIError(
                "obtain isv tenant access token，failure information:%s" % resp
            )
        tenant_access_token = TenantAccessToken(
            resp.data["tenant_access_token"], resp.data["expire"]
        )
    ...
    def get_internal_tenant_access_token(self):
        ...
        if resp.code != ERR_CODE_OK:
            raise APIError(
                "obtain internal tenant access token，failure information:%s" % resp
            )
...
def resolve_path(path, path_variables):  # type: (str, Dict[str, str]) -> str
    ...
    for i in range(len(splatted)):
        if splatted[i].startswith(":"):
            ...
            if v == "":
                raise RuntimeError(
                    "http path:{}, param name `{}` not found value".format(path, k)
                )
            splatted[i] = v
